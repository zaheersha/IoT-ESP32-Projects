...
#define BOT_TOKEN "5320062699:AAFEzAqmX-KvlRTlTX657XMJ_izp-UKk9sw"
...
void handleNewMessages(int numNewMessages)
{
  ...
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if (text == "/lighton")
    {
      digitalWrite(lightPin, HIGH); // turn the light on (HIGH is the voltage level)
      lightStatus = 1;
    ...
    {
      lightStatus = 0;
      digitalWrite(lightPin, LOW); // turn the light off (LOW is the voltage level)
...
void setup()
{
  ...
  delay(10);
  digitalWrite(lightPin, LOW); // initialize pin as off (active LOW)
